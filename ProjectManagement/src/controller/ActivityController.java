...
//			managementApp.getState().currentActivity().addHoursToWorker((int) (sliderValue*2));
	...
	private void updateTable() {
		...
		ArrayList<Worker> ws = State.getInstance().currentActivity().getWorkerList();
		for(int i = 0; i < ws.size(); i++) {
			list.add(new ActivityUserTable(i,managementApp.getState().currentActivity().getWorkerList().get(i).getUsername(),ws.get(i).getHoursOnActivity(managementApp.getState().currentActivity())/2.0));
		}
